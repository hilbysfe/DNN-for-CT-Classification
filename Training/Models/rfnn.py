...
class RFNN(object):
	...
	def __init__(self, sigmas, kernels, maps, n_classes=2, bases_3d = False, dropout_rate_conv=.2, dropout_rate_hidden=0.7, is_training = True):
		...
		self.alphas_L1 = init_alphas(maps[0],3,bases_L1, name="L1_alphas")
		self.alphas_L2 = init_alphas(maps[1],maps[0],bases_L2, name="L2_alphas")
		self.alphas_L3 = init_alphas(self.n_classes,maps[1],bases_L3, name="L3_alphas")
		
		self.biases_L1 = init_biases(maps[0], name='ConvLayer1/biases')
		self.biases_L2 = init_biases(maps[1], name='ConvLayer2/biases')
		self.biases_L3 = init_biases(self.n_classes, name='ConvLayer3/biases')
	...
	def inference_2d(self, X):
		...
		# ==== Layer 1 ====				
		layer1 = [ _conv_layer_2d_with_kernel_and_bias(
					input=X,
					biases=self.biases_L1,
					kernel=w_L1[i],
					strides=[1,1,1,1],
					padding='SAME',
		...
		# ==== Layer 2a ====			
		layer2 = [ _conv_layer_2d_with_kernel_and_bias(
					input=net,
					biases=self.biases_L2,
					kernel=w_L2[i],
					strides=[1,1,1,1],
					padding='SAME',
					is_training=self.is_training)
		...
		# ==== Layer 2b ====		
		layer3 = [ _conv_layer_2d_with_kernel_and_bias(
					input=net,
					biases=self.biases_L3,
					kernel=w_L3[i],
					strides=[1,1,1,1],
					padding='SAME',
					is_training=self.is_training)
