...
training_points, test_points, validation_points = utils_final.read_dataset(
	r"D:\Adam Hilbert\Data\data_binaries\MIP2D\tici_imp")
	...
	with tf.Session(config=config) as sess:
		...
		for f in range(FLAGS.xvalidation_folds):
			...
			for trial in range(FLAGS.trials):
				...
				for i in range(FLAGS.max_epochs):
					...
					# ------------ VALIDATION & PRINT -------------
					tot_loss_entropy = cost.eval(
							feed_dict={x: dataset.Validation.vars, y: dataset.Validation.labels, prob: 1.0})
					print("Validation CE: %.4f, Training CE: %.4f" % (tot_loss_entropy, avg_loss))

					# Save bets model so far
					if tot_loss_entropy < min_loss and i > FLAGS.max_epochs * 0.5:
		...
		for f in range(FLAGS.xvalidation_folds):
			...
			for k in range(iters):
				...
				_, _, softmax = sess.run([optimizer, cost, probs],
											feed_dict={x: dataset.Test.vars, y: dataset.Test.labels, prob: 1})

				pred_temp = tf.equal(tf.argmax(output_layer, 1), tf.argmax(y, 1))
				accuracy = tf.reduce_mean(tf.cast(pred_temp, "float"))

				# Compute ROC curve and ROC area for each class
				fpr, tpr, _ = roc_curve(dataset.Test.labels[:,1], softmax[:,1])
				tprs.append(interp(mean_fpr, fpr, tpr))
				tprs[-1][0] = 0.0

				auc_k = auc(fpr, tpr)
				aucs.append(auc_k)
				accs.append(accuracy.eval({x: dataset.Test.vars, y: dataset.Test.labels}))
