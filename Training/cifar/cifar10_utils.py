...
def preprocess_cifar10_data(X_train_raw, Y_train_raw, X_test_raw, Y_test_raw):
  ...
  """
	...
	std_0 = np.std(X_train[:, :, :, 0])
	std_1 = np.std(X_train[:, :, :, 1])
	std_2 = np.std(X_train[:, :, :, 2])

	for i in range(np.shape(X_train)[0]):
		X_train[i, :, :, 0] = np.divide(X_train[i, :, :, 0] - mean_0, std_0)
		X_train[i, :, :, 1] = np.divide(X_train[i, :, :, 1] - mean_1, std_1)
		X_train[i, :, :, 2] = np.divide(X_train[i, :, :, 2] - mean_2, std_2)
	for i in range(np.shape(X_test)[0]):
		X_test[i, :, :, 0] = np.divide(X_test[i, :, :, 0] - mean_0, std_0)
