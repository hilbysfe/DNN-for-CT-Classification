...
def MIP_2D(rootSource, patient, rootTarget):
	# if not os.path.exists(os.path.join(rootTarget, patient)):
	try:

		input_data = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(rootSource, patient)))
		...
		# os.makedirs(os.path.join(rootTarget, patient))
		sitk.WriteImage(mip_image, os.path.join(rootTarget, patient))
