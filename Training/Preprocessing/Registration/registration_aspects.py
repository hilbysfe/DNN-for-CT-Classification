...
FAILED_FOLDER = '/home/hilbysfe/DATA/SUPERVISED/REGISTRY/NCCT/ASPECTS_FAILED/'
...
def register(patient, rootSource, rootTarget, atlasScan, atlasMask, aspects, cta = False, scanBrain=""):
	...
	for thickness in thicknesses:
		try:
			...
			if os.path.exists(os.path.join(FAILED_FOLDER,patient,str(thickness))):
				continue
			...
			if os.path.exists(bsplineAspects):
				continue
			...
			print(patient + ' ' + str(thickness) + ' done.')
		except:
			print(patient + ' ' + str(thickness) + ' failed.')
	
DATADIR = r'/home/hilbysfe/DATA/SUPERVISED/REGISTRY/NCCT/ST_BL_ALL/'
OUTDIR = r'/home/hilbysfe/DATA/SUPERVISED/REGISTRY/NCCT/ASPECTS_ALL_PART2/'
...
#	patients = os.listdir(DATADIR)
	with open('/home/hilbysfe/DATA/SUPERVISED/REGISTRY/NCCT/log_cloud.txt') as f:
		patients = f.readlines()
	patients = [x.strip() for x in patients]
	with Pool() as p:
		p.starmap(register,zip(patients, repeat(DATADIR),repeat(OUTDIR),repeat(ATLAS),repeat(BRAINATLAS),repeat(ASPECTS)))
#	register(
#				patient='R0005',
#				rootSource=DATADIR,
#				rootTarget=OUTDIR,
