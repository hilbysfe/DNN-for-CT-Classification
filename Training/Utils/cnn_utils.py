...
def batch_norm_wrapper(inputs, is_training, is_conv, decay=0.999):
	...
	scale = tf.get_variable(name='scale', initializer=tf.constant_initializer(1.0), shape=[inputs.get_shape()[-1]],
							dtype=tf.float32)
	beta = tf.get_variable(name='beta', initializer=tf.constant_initializer(0.0), shape=[inputs.get_shape()[-1]],
						   dtype=tf.float32)
	pop_mean = tf.get_variable(name='pop_mean', initializer=tf.constant_initializer(0.0),
							   shape=[inputs.get_shape()[-1]], trainable=False, dtype=tf.float32)
	pop_var = tf.get_variable(name='pop_var', initializer=tf.constant_initializer(1.0), shape=[inputs.get_shape()[-1]],
