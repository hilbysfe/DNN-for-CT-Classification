...
class ConvNet(object):
	...
	def __init__(self, n_classes = 10, is_training = True):
		"""
		...
		self.act				= tf.nn.relu
#		self.regularizer =		tf.contrib.layers.l2_regularizer(0.15)
...
#		self.normalizer			= tf.nn.lrn
	...
	def _dropout_layer(self, input, drop_rate, name):
		with tf.variable_scope(name):
			...
			logits = tf.nn.dropout(input, keep_prob, name='Activation')
...
#						name='NormLayer2')
			...
			with tf.variable_scope('Flatten'):
				fshape = pool2.get_shape()
				dim = fshape[1].value*fshape[2].value*fshape[3].value
				flatten = tf.reshape(pool2, [-1, dim], name='Activation')
